...
#
# Description: M3 - temperature and body size dependant population divergence model of the ESH
#
...
#environmental_ranges = list("temp" = c(-38,  34), "prec"=NA) # will work for both inputs
environmental_ranges = list("temp" = NA, "prec"=NA) # will optimise for single input
...
get_divergence_factor <- function(species, cluster_indices, landscape, config) {
  ...
  for(i_x in 1:length(cluster_indices_order)){
    ...
    mean_body_size_temp_i <- mean(c(mean_temp_i, mean_body_size_i), na.rm=T)
    ...
    for(j_x in 1:length(cluster_indices_order)){
      ...
      mean_body_size_temp_j <- mean(c(mean_temp_j, mean_body_size_j), na.rm=T)
      divergence_matrix[i_x, j_x] <- ((sum(mean_body_size_temp_i, mean_body_size_temp_j)/2))^lambda
    }
  }
